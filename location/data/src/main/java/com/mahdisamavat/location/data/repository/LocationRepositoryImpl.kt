...
class LocationRepositoryImpl @Inject constructor(
    ...
    override suspend fun getAllLocations(): Result<List<Location>> = withContext(ioDispatcher) {
        try {
            ...
            logger.i(TAG, "Retrieved ${locations.size} locations from database")
            Result.success(locations)
        } catch (e: Exception) {
    ...
    override suspend fun getLatestLocation(): Result<Location?> = withContext(ioDispatcher) {
        try {
            ...
            if (location != null) {
                logger.i(TAG, "Retrieved latest location: id=${location.id}, timestamp=${location.timestamp}")
            } else {
    ...
    override suspend fun storeLocation(location: Location): Result<Long> = withContext(ioDispatcher) {
        try {
            ...
            logger.i(TAG, "Location stored successfully with id=$id")
            Result.success(id)
        } catch (e: Exception) {
    ...
    override suspend fun storeAndroidLocation(androidLocation: android.location.Location): Result<Long> = withContext(ioDispatcher) {
        try {
            ...
            logger.i(TAG, "Android location stored successfully with id=$id")
            Result.success(id)
        } catch (e: Exception) {
